            ...
            <table class="min-w-full">
                <thead>
                    <tr class="border-gray-200 border-y dark:border-gray-700">
                        ...
                        <th scope="col" class="px-4 py-3 font-normal text-gray-500 text-start text-theme-sm dark:text-gray-400">Usernamne</th>
                        <th scope="col" class="px-4 py-3 font-normal text-gray-500 text-start text-theme-sm dark:text-gray-400">Password</th>
                        <th scope="col" class="px-4 py-3 font-normal text-gray-500 text-start text-theme-sm dark:text-gray-400">Jumlah Request</th>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                  ...
                  <?php
                    $jlm_req = DB::table('request')->where('id_cus_req', $cs->id_users )->count();
                  ?>
                    <tr>
                        <td class="py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <div class="shrink-0 w-8 h-8">
                                    <img class="w-8 h-8 rounded-full" src="images/user/profil.jpg" alt="">
                                </div>
                                <div class="ml-4">
                                    <div class="text-sm font-medium text-gray-900 dark:text-white">{{ $cs->name }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-4 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-500 dark:text-gray-400">{{ $cs->username }}</div>
                        </td>
                        <td class="px-4 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-500 dark:text-gray-400">{{ $cs->password_asli }}</div>
                        </td>
                        <td class="px-4 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-500 dark:text-gray-400">{{ $jlm_req }}</div>
                        </td>
                        <td class="px-4 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-500 dark:text-gray-400">{{ $cs->status_users }}</div>
                        </td>
                        <td class="px-4 py-4 whitespace-nowrap">
                            <div class="flex items-center justify-center gap-3">
                                <!-- View -->
                                <button type="button" class="text-blue-500 hover:text-blue-700 buka_modal" data-cek="detail" data-cek="detail" data-id="{{ $cs->id_users }}" data-name="{{ $cs->name }}" data-username="{{ $cs->username }}" data-pw="{{ $cs->password_asli }}" data-status="{{ $cs->status_users }}">
                                ...
                                <!-- Edit -->
                                <button type="button" class="text-yellow-500 hover:text-yellow-700 buka_modal"  data-cek="edit" data-id="{{ $cs->id_users }}" data-name="{{ $cs->name }}" data-username="{{ $cs->username }}" data-pw="{{ $cs->password_asli }}" data-status="{{ $cs->status_users }}">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <!-- Delete -->
                                <form method="POST" action="{{ route('customer.destroy', $cs->id_users) }}"
...
<div id="view_customer" class="hidden fixed inset-0 z-[999999] flex items-center justify-center bg-black/50">
        ...
        <form method="POST" action="{{ route('customer.store') }}">
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" placeholder="Masukkan nama lengkap" class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
                </div>
                ...
                <div>
                    ...
                    <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                        <select name="status_users" id="status"
